...
% 
P = eye(size(A_1,1)) + MW;
EA = @(T) V1*diag(exp(diag(D1)*T))*inv(V1);
%
...
%  T = 0.0:0.01:0.1;
 ...
 equi_type =1;
 for i=1:length(T)
%      [V_sign(i),LOCI(:,i),MAX(i),F_1(i)] = solve_classification(T(i),P,A_1,sign_V);
...
function [sign,LOCI,Max,F_1] = solve_classification(T,R,A,sign_V)
M = R*expm(A*T)-eye(5);
PA = R*expm(A*T);
