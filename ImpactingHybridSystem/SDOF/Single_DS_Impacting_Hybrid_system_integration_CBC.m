...
A = real(A);
% define the linear ode system with contro
% y0_ = [1;8];
e_d = [1;0];
e_v = [0;1];
kd =1; kv =1;
f_ls= @(t,y,te,y0_) A*y + e_v*kd*C*(EA(t-te)*y0_-y)+e_v*kv*C*A*(EA(t-te)*y0_-y);
% if R(x) =x + w(x)*v(x) = (I + W*C*A)*x = R*x
...
converge=0;
y0_ = InitCond;
while tout(end)<tfinal && converge ==0
    ...
    if BeStuck==false
        ...
        [t,y,te,ye,ie] = ode45(@(t,y) f_ls(t,y,te,y0_),...
            timespan,y0,options);
    ...
    if (length(teout)>1)
    ye_cbc=EA(teout(end)-teout(end-1))*y0_;
    else
    ye_cbc= EA(teout(end)-0)*y0_;    
    end
    t0=t(end);
    x=y(end,:)';
    [tstart,y0,BeStuck,tout,yout]=Post_impact(x,A,R,C,t0,tfinal,tout,yout,fs,ye_cbc);
   

    % to tell if the system converges to a LCO and cease the simulation
    if  size(yeout0,1) >3 
        error1 = norm(yeout(end,:)-yeout(end-1,:))/norm(yeout(end,:));
        error2 = norm(yeout(end-1,:)-yeout(end-2,:))/norm(yeout(end-1,:));
        error3 = norm(yeout(end-2,:)-yeout(end-3,:))/norm(yeout(end-2,:));
        if error1< 1e-4 && error2< 1e-4 && error3< 1e-4 
            converge =1;
        end
    end
...
function [tstart,y0,BeStuck,tout,yout]=Post_impact(x,A,R,C,t0,tfinal,tout,yout,fs,ye_cbc)
Fx=A*x;
vx=C*Fx;
ax=C*A*A*x;
if vx>0 && t0<tfinal
