% 1DOF function
% bifurcation Diagram for SDOF impact oscillator
% epsilon: the varying parameter
% mu: the damping ration
% r: impact coeeficient of restitution
% with unstable equilibrium
clc
clear
close all
% mu=0.1;
...
%             [dt2,~]=instant_solve(beta,dot_beta,mcoef,eigen_value,t0);
        ...
        dt_temp(dt_temp(:,1)<0.1,:)=[];
        dt_temp(dt_temp(:,1)>6.18,:)=[];
        dt_temp(abs(X(dt_temp(:,1)))>1e-3,:)=[];
