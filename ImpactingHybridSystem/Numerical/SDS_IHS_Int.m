%% Single Discontinuous Surface Impacting Hybrid System Integration
...
% tol: passing the tollerance

% add the tol argument
% Modified by Peter Tang, 23rd/Aug/2022
% 
function [tout,yout,yeout0,teout,yeout,ieout]=...
    SDS_IHS_Int(A,R,C,InitCond,tspan,fs,equi_type,tol)

% tol_v: the threshold to end the chattering sequence
tol_v = tol.tol_v;
...
if abs(C*y0-equi_type)<1e-3 && C*A*y0<0
    [tstart,y0,BeStuck,tout,yout]=Post_impact(y0,A,R,C,tstart,tfinal,tout,yout,fs,tol_v);
end
% define options
refine = 1;
...
while tout(end)<tfinal
    ...
    t0=t(end);
    x=y(end,:)';
    [tstart,y0,BeStuck,tout,yout]=Post_impact(x,A,R,C,t0,tfinal,tout,yout,fs,tol_v);
...
function [tstart,y0,BeStuck,tout,yout]=Post_impact(x,A,R,C,t0,tfinal,tout,yout,fs,tol_v)
Fx=A*x;
vx=C*Fx;
ax=C*A*A*x;
if vx>0 && t0<tfinal
    ...
    rx = abs(1+C*A*Wx);
    if abs(vx)<tol_v && ax<0 && t0<tfinal
