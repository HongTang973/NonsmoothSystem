...
h11 = plot(T/pi,F_1,'r-','linewidth',1.2,'displayname','indication function');
hold on
%  plot( T/pi,V_sign,'-','linewidth',1.2,'displayname','Sign of Velocity')
h12 =plot([a b],[0 0],'b-','displayname','zero line');
...
% B0 =B(P_l3,V_l3);
% 
% [A_c,B_c,PZ] = companion_form(A,-B0)
% % eig(A_c)
...
if ~isempty(T_chosen)
    ...
    for i=1:length(T_chosen)
        ...
        figure(1)
        plot(T_chosen(i)/pi,F1_chosen(i),'bo')
        %,'displayname',['the ',num2str(i),'th candidate']
    end
    figure(1)
    xlim([0 right]/pi)
...
if ~isempty(T_chosen)
    ...
    for i =1:length(T_chosen)
    InitCond = LCO(:,i);
    sign_ = sign(C*A*InitCond)
    [tout,yout,yeout0,teout,yeout,ieout]=...
        Single_DS_Impacting_Hybrid_system_integration(A,R,C,sign_*InitCond,tspan,fs,sign_*equi_type);
