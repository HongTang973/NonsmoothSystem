...
% ------SubCASE 1: LAMBDA_3 = -1;  lambda_1 & lambda_2 are REAL ---------%
...
%     b3 = 0.2;
    ...
    try
        [A,R,C]= Matrices_3D_IHS_Gform(p,'Case1');
        ...
        if ~isempty(T_chosen)
            ...
            Monte_3D(i,:) = {p.lambda_1,p.lambda_2,p.lambda_3,p.b2,p.b3, length(T_chosen), ...
                T_chosen,F_value, V_sign_LCO, MAX , datetime};
            
        else
            
            Monte_3D(i,:) = { p.lambda_1,p.lambda_2,p.lambda_3,p.b2,p.b3, 0, ...
                0 ,1, 2, -5 , datetime};
        ...
        disp(['error at',num2str(p.lambda_1),',',num2str(p.lambda_2),',',num2str(p.b3)])
...
save('Monte_3D_case1.mat','Monte_3D')
