function [A,B,C,R,T_2_det] = par2NForm_Lienard_David_CD2(par)
% function [A,B,C,R,T_2_det,T_2_d_det_dt] = par2NForm_Lienard_David_CD2(par)
...
lambda_3 = par(3);
b2       = par(4); % the eta dependence as required to unfold the SN
b3       = par(5);
...
B = [0; b2; b3];
C = [1,0,0];
A = [a1 1 0;a2 0 1; a3 0 0] + A_1*C*real(par(7)); % the mu dependence to unfold BEB
%
R = eye(length(C)) - B*C*A;
%>
% now it is unclear about the explicit formula
...
T_2_det = @(t) det(R*expm(A*t) - eye(length(C)));
end