function [poly_coeff_CtrErr, par_coeff_CtrErr,bilinear_coeff_CtrErr] = IHS_PMP_3D_exapnsion_in_Eigenspace(prob, sys_vec)
...
% *********************************************************%
%---------------------------AIM----------------------------%
% Calculate the Poincare map expansions numerically around the fixed point
% in the coordinate of the eigenspace, and get the normal form of the PD/SN %
% of the fixed point                                       %
% [X;Y] = P^-1*[x;y]; [X_p; Y_p] = F(X,Y) = P^-1*f(P*[X;Y])
% *********************************************************%
...
% now v, w should be orthoganal basis
v = [1;0];
w = [0;1];
...
while delta > min_delta
    ...
    d_new      = v'*(P\( phi_p2(ind) - phi_m2(ind) - 2*phi_p1(ind) + 2*phi_m1(ind) ))/2/delta^3;
...
for j = 1:dim_par
    ...
    while delta > min_delta
        ...
        %         [x_p; T_p; sys_par; x_p - 2*S*delta* v; T]);
        %> approximate the first derivative f' = [f(x + h) - f(x-h)]/2/h;
        FO_new = v'*(P\( phi_p1(ind) - phi_m1(ind)))/2/delta;

        %> approximate the first derivative f'' = [f(x + h) + f(x-h) - 2*f(x)]/h^2;
        SO_new      = v'*(P\( phi_p1(ind) +  phi_m1(ind) - 2*x_p(ind)))/delta^2;
    ...
    for j = 1:dim_par
        ...
        while delta > min_delta
            ...
            phi_p1_p1 = General_IHS_P1_Composed_Map(prob, ...
                [x_p; T_p; sys_par + delta*par_selection; x_p + S*P*delta* v ; T]);
            %  ------- Phi(sys_par - delta, x_p - delta) -------
            ...
            phi_m1_p1 =  General_IHS_P1_Composed_Map(prob, ...
                [x_p; T_p; sys_par - delta*par_selection; x_p + S*P*delta* v ; T]);
            %> approximate the first derivative f' = [f(x + h) - f(x-h)]/2/h;
            tmp         = P\( phi_p1_p1(ind) +  phi_m1_m1(ind) - phi_p1_m1(ind) - phi_m1_p1(ind))/4/delta^2;
            FO_new      = v'*tmp;
